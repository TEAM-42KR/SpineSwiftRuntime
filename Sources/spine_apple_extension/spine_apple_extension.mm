...
#include "spine_apple_extension/spine_apple_extension.h"
		...
		*length = 0;
		@autoreleasepool {
			...
			if (url) {
				...
				if (error) {
					os_log_error(logger, "Error reading file size: %@ errorCode: %ld", error, static_cast<long>(error.code));
				} else if (data == nil) {
...
#define P_VALUE(key) @ #key : @(page.key),
		...
		buffer[@"kSpineTextureProperty"] = @{
			...
			P_VALUE(width) P_VALUE(height) P_VALUE(index) P_VALUE(pma) P_VALUE(uWrap) P_VALUE(vWrap) P_VALUE(magFilter) P_VALUE(minFilter)
				P_VALUE(format)
		};
