...
#include "spine_apple_extension/extensions/spine_ns_dictionary_texture_loader.h"
...
#define page_s(key) @ #key : page.key.isEmpty() ? @"" : [NSString stringWithUTF8String:page.key.buffer()],
			@autoreleasepool {
				...
				dict[@"kSpineAtlasPageProperty"] = @{
					page_s(name) @"texturePath": [NSString stringWithUTF8String:path.buffer()],
					page_v(format) page_v(minFilter) page_v(magFilter) page_v(uWrap) page_v(vWrap) page_v(width) page_v(height) page_v(pma)
						page_v(index)
				};
