...
public final class SpineMathUtils: NSObject {
    ...
    public static func transformPath(
        ...
        let vector = CGVector(dx: alignment.x, dy: alignment.y)
    ...
    public static func measurePath(_ head: spine_render_command) -> CGPath {
        ...
        for cmd in commandEntries {
            ...
            let indexBuffer = UnsafeBufferPointer<UInt16>(
                start: spine_render_command_get_indices(cmd), count: .init(spine_render_command_get_num_indices(cmd)))

            let vertexBuffer = UnsafeRawBufferPointer(
