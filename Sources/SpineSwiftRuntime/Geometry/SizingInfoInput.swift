...
public struct SizingInfoInput: Hashable, BitwiseCopyable, Sendable {
    ...
    func generateOutput() -> SizingInfoOutput {
        SpineMathUtils.translateSize(
            textureBoundInPoint: .init(origin: offSet, size: displaySize), contentScale: displayScale, boundOfDrawable: spineNativeBound,
            alignment: alignment, contentMode: contentMode)
    }
}
